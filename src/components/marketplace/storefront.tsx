    ...
    <section className="border-y border-primary/10 bg-gradient-to-br from-canvas via-background to-primary/[0.045]">
      <div className="shell py-14 sm:py-18">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-end">
          <div><span className="inline-flex rounded-full bg-primary/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-primary">Verified wholesale network</span><h1 className="mt-5 text-4xl font-extrabold tracking-tight text-ink sm:text-5xl"><span className="mr-3 inline-block size-2 rounded-full bg-primary align-middle"/>Find the right wholesaler for every aisle</h1><div className="mt-5 h-px w-16 bg-primary"/><p className="mt-5 max-w-2xl leading-7 text-muted-foreground">Search by category, review business credentials, and request a connection. Wholesale prices and products unlock only after approval.</p><div className="group relative mt-7 max-w-2xl"><Search className="pointer-events-none absolute left-5 top-1/2 size-5 -translate-y-1/2 text-muted-foreground transition-colors group-focus-within:text-primary"/><input aria-label="Search wholesaler, city or category" value={q} onChange={e=>setQ(e.target.value)} placeholder="Search wholesaler, city or category" className="h-14 w-full rounded-full border border-border bg-card pl-13 pr-5 text-sm text-ink shadow-sm outline-none transition-all placeholder:text-muted-foreground focus:border-primary focus:ring-4 focus:ring-primary/10"/></div></div>
          <div className="grid grid-cols-3 gap-3">{stats.map(([Icon,value,label])=><div key={label} className="rounded-2xl border border-border bg-card p-4 shadow-sm"><Icon className="size-5 text-primary"/><strong className="mt-4 block text-2xl text-ink">{value}</strong><span className="mt-1 block text-xs font-medium text-muted-foreground">{label}</span></div>)}</div>
        </div>
      </div>
    </section>
    <section className="shell py-10 sm:py-12"><div className="flex items-end justify-between gap-4"><div><h2 className="text-xl font-extrabold text-ink">Shop by category</h2><p className="mt-1 text-sm text-muted-foreground">Choose a category to see specialists</p></div><Button asChild variant="ghost" size="sm"><Link to="/categories">View all <ChevronRight/></Link></Button></div><div className="mt-5 flex items-center gap-3"><button type="button" aria-label="Scroll categories left" onClick={()=>chipListRef.current?.scrollBy({left:-220,behavior:"smooth"})} className="grid size-9 shrink-0 place-items-center rounded-full border bg-card text-muted-foreground shadow-sm transition-colors hover:border-primary hover:text-primary"><ChevronLeft className="size-4"/></button><div ref={chipListRef} className="flex flex-1 gap-2 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:none]"><Button className="shrink-0 rounded-full px-4 shadow-sm" size="sm" variant={category==="All"?"default":"outline"} onClick={()=>setCategory("All")}>All wholesalers</Button>{categories.map(c=><Button className="shrink-0 rounded-full border-border bg-card px-4 transition-colors hover:border-primary hover:bg-primary/5 hover:text-primary" key={c.id} size="sm" variant={category===c.name?"default":"outline"} onClick={()=>setCategory(c.name ?? "")}>{c.name}</Button>)}</div><button type="button" aria-label="Scroll categories right" onClick={()=>chipListRef.current?.scrollBy({left:220,behavior:"smooth"})} className="grid size-9 shrink-0 place-items-center rounded-full border bg-card text-muted-foreground shadow-sm transition-colors hover:border-primary hover:text-primary"><ChevronRight className="size-4"/></button></div></section>
    <section className="shell"><div className="flex flex-wrap items-center justify-between gap-3 border-y py-5 text-sm"><span className="font-semibold text-ink">{shown.length} verified match{shown.length===1?"":"es"} {category!=="All"?`for “${category}”`:"across all categories"}</span><span className="flex items-center gap-2 text-muted-foreground"><VerifiedUser className="size-4 text-success"/>Catalogue access stays private until connected</span></div><div className="mt-8 grid items-stretch gap-6 md:grid-cols-2 xl:grid-cols-3 xl:gap-8">{shown.map((w,index)=><div key={w.id} className="motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2" style={{animationDelay:`${index*50}ms`}}><WholesalerCard wholesaler={w} premium/></div>)}</div>{shown.length===0&&<div className="mt-8 border border-dashed bg-card p-12 text-center"><Search className="mx-auto size-9 text-muted-foreground"/><h2 className="mt-4 font-bold text-ink">No matching wholesalers</h2><p className="mt-2 text-sm text-muted-foreground">Try another category, location or business name.</p><Button className="mt-5" variant="outline" onClick={()=>{setQ("");setCategory("All")}}>Clear search</Button></div>}</section>
  </main>
}
      ...
      <main className="shell py-16 sm:py-24">
        <div className="mx-auto max-w-xl rounded-2xl border border-border bg-card p-10 text-center shadow-md">
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            {w.name}’s trade catalog, wholesale pricing, and inventory details are restricted to approved retailer partners.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            {relationship === "Request Access" ? (
              <Button size="lg" className="rounded-xl font-bold" onClick={() => requestAccess(w.id)}>
                Request connection
              </Button>
            ) : (
              <Button size="lg" variant="secondary" className="rounded-xl font-bold" onClick={() => approveRequest(w.id)}>
                Approve connection (demo)
              </Button>
            )}
